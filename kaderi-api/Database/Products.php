...
class Products
{
	 ...
	 * @return array|bool
	 */
	public function getProducts($store_id, $user){
		$products_list = array();
		try{
            ...
            if ($user < 1) {
                $products = $this->handle->prepare("SELECT * FROM products WHERE (store_id=:store_id AND available_quantity > 0) ORDER BY item_name ASC ");
            }
